...
export default function Navbar() {
  ...
  return (
    <nav 
      className={`fixed top-0 w-full z-50 transition-all duration-500 ${
        scrolled 
          ? "py-3 bg-white/90 dark:bg-[#030712]/90 backdrop-blur-xl border-b border-slate-200 dark:border-white/10 shadow-lg" 
          : "py-5 bg-transparent"
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8">
          <div className="flex gap-8 border-r border-slate-200 dark:border-white/10 pr-8">
            {navLinks.map((link, i) => (
              <a 
                key={link.name}
                href={link.href}
          ...
          <div className="flex items-center gap-6">
            ...
            <Link 
              href="/login" 
              className="text-[11px] font-mono font-bold uppercase tracking-widest text-slate-600 dark:text-gray-400 hover:text-blue-500 transition-colors"
            >
              Login
            </Link>
            ...
            <Link 
              href="/register" 
              className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-[11px] font-mono font-bold uppercase tracking-widest rounded-lg transition-all shadow-lg shadow-blue-500/20 active:scale-95"
            >
              Join Academy
            </Link>
            <ThemeToggle />
        ...
        <div className="md:hidden flex items-center gap-4">
          <ThemeToggle />
          <Link href="/register" className="p-2 text-blue-500">
