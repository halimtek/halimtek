...
export default function RegisterForm() {
  ...
  const [formData, setFormData] = useState({
    fullName: "",
    email: "",
    password: "", // Required by your FastAPI UserRegister model
    projectVision: ""
  });
  ...
  const isFormComplete = 
    formData.fullName.trim() !== "" && 
    formData.email.trim() !== "" && 
    formData.password.length >= 8 && // Validation for secure access keys
    formData.projectVision.trim() !== "" && 
    selectedTracks.length > 0;
  ...
  const handleInitialize = async (e) => {
    ...
    setLoading(true);
    try {
      // Aligning payload with UserRegister Pydantic Model in main.py
      const payload = { 
  ...
  return (
    <motion.div 
      initial={{ opacity: 0, x: 30 }}
      animate={{ opacity: 1, x: 0 }}
      className="w-full max-w-2xl z-40"
    >
          ...
          <AnimatePresence mode="wait">
            {!isSubmitted ? (
              ...
              >
                ...
                <p className="text-slate-400 text-sm leading-relaxed mb-10 max-w-sm mx-auto font-mono">
                  Your engineering profile has been synced with MongoDB. Check your inbox for the [PENDING_REVIEW] notification.
                </p>
                <div className="p-4 bg-white/5 rounded-2xl border border-white/5 inline-block font-mono">
                  <p className="text-[9px] text-slate-500 uppercase tracking-widest">
                    REFERENCE_ID: {Math.random().toString(36).toUpperCase().substring(7)}
                  </p>
                </div>
              </motion.div>
            )}
