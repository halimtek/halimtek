...
// If not found, it defaults to your new Vercel backend URL.
const API_BASE = (process.env.NEXT_PUBLIC_API_URL || "https://halimtek-l9lo.vercel.app").replace(/\/$/, "");
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    // UPDATED: Fetching from Vercel instead of localhost
    fetch(`${API_BASE}/admin/candidates?status=all`)
      .then(res => res.json())
      .then(data => {
        // Simple check to ensure data is an array
  ...
  const approve = async (id: string) => {
    setProcessing(id);
    try {
      // UPDATED: Patching to Vercel instead of localhost
      ...
      <div className="flex flex-1 z-10">
        ...
        <div className="flex-1 p-8 overflow-auto">
          ...
          <div className="grid md:grid-cols-3 gap-6 mb-10">
            ...
            <div className="col-span-3 bg-white/5 border border-white/10 p-6 rounded-2xl">
              ...
              <ResponsiveContainer width="100%" height={150}>
                <LineChart data={chartData}>
                  <Line type="monotone" dataKey="value" strokeWidth={3} stroke="#2dd4bf" />
                </LineChart>
              </ResponsiveContainer>
            </div>
          ...
          <div className="space-y-4">
            {filtered.length > 0 ? (
                  ...
                  <div className="flex gap-4 items-center">
                    {user.status==="pending" && (
                      <button
                        ...
                        className="px-4 py-2 bg-teal-400 text-black rounded-lg font-bold disabled:opacity-50"
                      >
              ...
              <p className="text-center text-slate-500 py-10">No candidates found in this category.</p>
            )}
          </div>
        </div>
        ...
        <div className="w-80 border-l border-white/10 p-6 bg-white/5 backdrop-blur-xl hidden lg:block">
          <h2 className="font-bold mb-6 flex items-center gap-2">
            <Activity className="text-teal-400 size-4" /> Live Activity
          </h2>

          <div className="space-y-4 text-sm text-slate-400">
            <ActivityItem text="System synchronized" />
            <ActivityItem text="Vercel Backend Linked" />
            <ActivityItem text="Monitoring active..." />
          </div>
        </div>
